...
// ============================================
// MČR & MSR SILNICE 2026 — JESENÍK
// ATT Investments x Enervit
// Route: Jeseník → Lipová-lázně → Ramzová → Jeseník
// ============================================

const ROAD_PATH = [];
const ROAD_LENGTH = 2000;
const TOTAL_DISTANCE = 52000; // 52 km
...
function generateRoadPath() {
    for (let i = 0; i <= 200; i++) {
        ...
        let x = 0;
        x += Math.sin(t * Math.PI * 5) * 18;
        x += Math.sin(t * Math.PI * 9) * 7;

        // Jeseníky mountain serpentines mid-route (Ramzová climb)
        if (t > 0.35 && t < 0.7) {
            const climbT = (t - 0.35) / 0.35;
            x += Math.sin(climbT * Math.PI * 7) * 22 * Math.sin(climbT * Math.PI);
        }

        // Descent serpentines
        if (t > 0.7) {
            const descT = (t - 0.7) / 0.3;
            x += Math.sin(descT * Math.PI * 5) * 18 * (1 - descT * 0.5);
        }

        let elevation = 0;
        if (t < 0.15) {
            // Jeseník town, gentle rise
            elevation = t * 60 / 0.15;
        } else if (t < 0.35) {
            // Towards Lipová-lázně, rolling
            elevation = 60 + (t - 0.15) * 90 / 0.2 + Math.sin(t * Math.PI * 7) * 15;
        } else if (t < 0.6) {
            // Climb to Ramzová pass — steep!
            const climbT = (t - 0.35) / 0.25;
            elevation = 150 + climbT * 420 * (1 + climbT * 0.4);
        } else if (t < 0.65) {
            // Ramzová summit plateau
            elevation = 570 + Math.sin((t - 0.6) * Math.PI * 20) * 8;
        } else {
            // Descent back towards Jeseník
            const descT = (t - 0.65) / 0.35;
            elevation = 570 - descT * 350 * (0.6 + descT * 0.4);
        }

        ROAD_PATH.push({ x, z, elevation: elevation / 10 });
    }
}
generateRoadPath();
...
// AI Competitors — Czech & Slovak championship riders
const competitors = [];
...
const COMPETITOR_NAMES = [
    'Mathias Vacek', 'Pavel Bittner', 'Adam Ťoupalík', 'Jan Hirt',
    'Michael Kukrle', 'Petr Vakoč', 'Tomáš Kopecký', 'Erik Baška'
];

// Enervit Products
const enervitProducts = [
    { name: 'Enervit C2:1 Gel', effect: 'energy', value: 35, color: 0xe30613, description: '+35 Energie', situation: 'low_energy' },
    ...
    { name: 'Enervit Power Sport', effect: 'recovery', value: 25, color: 0xf1cc7e, description: '+25 Energie postupně', situation: 'recovery' }
];

// Three.js
let scene, camera, renderer;
let cyclist, terrainMeshes = [];
let pickups = [];
let trees = [];
let roadSegments = [];
...
function createRoad() {
    ...
    const yellowLineMaterial = new THREE.MeshBasicMaterial({ color: 0xf1cc7e });
...
function createCyclist() {
    // ATT Investments navy jersey
    cyclist = createCyclistModel(0x00205b);
    scene.add(cyclist);
}
...
function createEnvironment() {
    ...
    // ATT navy + gold arches
    createArch(ROAD_PATH[0].x, ROAD_PATH[0].z + 5, ROAD_PATH[0].elevation, 'START — JESENÍK');

    const lipovaIndex = Math.floor(ROAD_PATH.length * 0.3);
    const ramzovaIndex = Math.floor(ROAD_PATH.length * 0.6);

    createArch(ROAD_PATH[lipovaIndex].x, ROAD_PATH[lipovaIndex].z, ROAD_PATH[lipovaIndex].elevation, 'LIPOVÁ-LÁZNĚ');
    createArch(ROAD_PATH[ramzovaIndex].x, ROAD_PATH[ramzovaIndex].z, ROAD_PATH[ramzovaIndex].elevation, 'RAMZOVÁ');

    const finishP = ROAD_PATH[ROAD_PATH.length - 1];
    createArch(finishP.x, finishP.z, finishP.elevation, 'CÍL — JESENÍK');

    for (let km = 5; km < 52; km += 5) {
        const progress = km / 52;
...
function createTree(x, z, baseElevation) {
    ...
    if (type < 0.65) {
        // Spruce — Jeseníky are heavily forested spruce
...
function createArch(x, z, elevation, text) {
    ...
    // ATT navy pillars with gold banner
    const pillarMat = new THREE.MeshLambertMaterial({ color: 0x00205b });
    ...
    const bannerMat = new THREE.MeshLambertMaterial({ color: 0xf1cc7e });
    const banner = new THREE.Mesh(new THREE.BoxGeometry(14, 1.5, 0.4), bannerMat);
    banner.position.set(0, 7, 0);
    arch.add(banner);

    const textBg = new THREE.Mesh(
        new THREE.BoxGeometry(12, 1, 0.1),
        new THREE.MeshBasicMaterial({ color: 0x00205b })
    );
...
function createKmMarker(x, z, elevation, km) {
    ...
    const sign = new THREE.Mesh(
        ...
        new THREE.MeshLambertMaterial({ color: 0x00205b })
    );
    sign.position.y = 2;
    marker.add(sign);
...
function createJesenikyMountains() {
    const mountainGroup = new THREE.Group();
    const ramzovaP = ROAD_PATH[Math.floor(ROAD_PATH.length * 0.6)];

    // Praděd — highest peak of Jeseníky (1491 m)
    const praded = new THREE.Mesh(
        new THREE.ConeGeometry(90, 140, 8),
        new THREE.MeshLambertMaterial({ color: 0x4a6b4a })
    );
    praded.position.set(ramzovaP.x + 60, ramzovaP.elevation + 70, ramzovaP.z - 120);
    mountainGroup.add(praded);

    // Snow patches near summit
    const snow = new THREE.Mesh(
        new THREE.ConeGeometry(22, 28, 8),
        new THREE.MeshLambertMaterial({ color: 0xeeeeff })
    );
    snow.position.set(ramzovaP.x + 60, ramzovaP.elevation + 126, ramzovaP.z - 120);
    mountainGroup.add(snow);

    // Praděd TV tower (iconic landmark)
    const tower = new THREE.Group();

    const base = new THREE.Mesh(
        new THREE.CylinderGeometry(4, 7, 12, 12),
    ...
    const middle = new THREE.Mesh(
        new THREE.CylinderGeometry(3.5, 4, 22, 12),
        new THREE.MeshLambertMaterial({ color: 0xdddddd })
    );
    middle.position.y = 23;
    tower.add(middle);

    const top = new THREE.Mesh(
        new THREE.ConeGeometry(4.5, 16, 12),
        new THREE.MeshLambertMaterial({ color: 0xcccccc })
    );
    top.position.y = 42;
    tower.add(top);
    ...
    const antenna = new THREE.Mesh(
        new THREE.CylinderGeometry(0.3, 0.3, 10, 6),
        new THREE.MeshBasicMaterial({ color: 0xff0000 })
    );
    antenna.position.y = 55;
    tower.add(antenna);

    tower.position.set(ramzovaP.x + 60, ramzovaP.elevation + 140, ramzovaP.z - 120);
    mountainGroup.add(tower);

    // Surrounding Jeseníky peaks
    const peakOffsets = [
        { angle: 0.5, dist: 180, height: 70 },
        { angle: 1.2, dist: 200, height: 55 },
        { angle: 2.0, dist: 160, height: 80 },
        { angle: 2.8, dist: 220, height: 60 },
        { angle: 3.8, dist: 170, height: 65 },
        { angle: 4.8, dist: 190, height: 50 },
        { angle: 5.5, dist: 210, height: 75 },
    ];

    peakOffsets.forEach(({ angle, dist, height }) => {
        const peak = new THREE.Mesh(
            new THREE.ConeGeometry(35 + Math.random() * 25, height, 6),
            new THREE.MeshLambertMaterial({ color: 0x3d5c3d })
        );
        peak.position.set(
            ramzovaP.x + Math.cos(angle) * dist,
            ramzovaP.elevation + height / 2,
            ramzovaP.z - 120 + Math.sin(angle) * dist * 0.5
        );
...
function showProductSelection(pickup) {
    ...
    } else if (gameState.progress > 0.35 && gameState.progress < 0.65) {
        situation = 'uphill';
        situationMessage = 'Stoupání na Ramzovou! Připrav se na kopec.';
...
function updateGame(delta) {
    ...
    // Jeseník elevation profile: ~440 m start, climb to ~780 m at Ramzová, descend back to ~440 m
    if (gameState.progress < 0.6) {
        gameState.elevation = 440 + (780 - 440) * (gameState.progress / 0.6);
    } else if (gameState.progress < 0.65) {
        gameState.elevation = 780;
    } else {
        gameState.elevation = 780 - (780 - 440) * ((gameState.progress - 0.65) / 0.35);
    }
    ...
    if (gameState.energy < 20) {
        ...
        energyBar.style.background = 'linear-gradient(90deg, #f1cc7e, #d4a84a)';
    } else {

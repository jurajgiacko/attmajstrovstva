...
(function () {
  ...
  function shareScore(state) {
    const score = computeScore(state);
    const text = `Jeseník dokončen v ${fmtTime(state.elapsed)} — ${score.total} bodů. ATT Investments × Enervit · MČR & MSR Silnice 2026. Zkus mě porazit: ${location.origin}`;
    if (navigator.share) {
      navigator.share({ title: 'ATT × Enervit — MČR & MSR Silnice 2026 Jeseník', text, url: location.origin })
        .catch(() => {});
    } else {

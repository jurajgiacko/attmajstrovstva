...
(function () {
  ...
  const monumentId = params.get('monument') || 'jesenik';
  ...
  let bgFromArt = 'jesenik-morning';
  let bgToArt   = 'jesenik-morning';
  let bgFadeT   = 1;            // 0 = fully showing fromArt, 1 = fully showing toArt
  let bgLastZoneArt = 'jesenik-morning';
  ...
  async function boot(opts = {}) {
    ...
    if (window.rcSprites && window.rcSprites.preload) {
      window.rcSprites.preload([
        ...
        'jesenik-morning', 'lipova-lazne', 'ramzova-village',
        'ramzova-climb', 'cervenohorske-sedlo', 'praded-vista', 'jesenik-finish'
      ]);
    }
